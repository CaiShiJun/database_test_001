...
public class MysqlJdbcController {
    ...
    @RequestMapping("/mysqlInsert10000")
    @ResponseBody
    public String mysqlInsert10000() {
        try {
            Connection conn = sqlExcute.getConnPs("mysql", "Jdbc");
            for(int i = 20000;i<100000;i++ ){
                String insertsql2 = "insert into GARTNER_USER_RECORD(id,username,password) values (?,?,?)";
                PreparedStatement preparedStatement = conn.prepareStatement(insertsql2);
                preparedStatement.setInt(1, i);
                UUID uuid = UUID.randomUUID();
                preparedStatement.setString(2, uuid.toString());
                uuid = UUID.randomUUID();
                preparedStatement.setString(3, uuid.toString());
                System.out.println("@RequestMapping(mysqlInsert10000)"+i);
                preparedStatement.executeUpdate();
            }
            PreparedStatement preparedStatement = conn.prepareStatement("Select max(id) from GARTNER_USER_RECORD");
            ResultSet rs = preparedStatement.executeQuery();
            while (rs.next()) {
                int queryResult = rs.getInt(1);
                System.out.println("@RequestMapping(mysqlInsert10000)"+queryResult);
            }
...
//            preparedStatement2.execute();
    ...
    public String getMysqlPs() {
        ...
        try {
            ...
            preparedStatement.setInt(1, 1);
            preparedStatement.setString(2, "PreparedStatement");
            preparedStatement.setString(3, getRandomString(250));
            preparedStatement.executeUpdate();
